...
- **重启恢复控制**：应用重启后自动恢复对上次投屏的暂停 / 停止 / 音量控制（控制走设备自身的 AVTransport；流本身随重启换端口失效，重新投屏即可）。
- **流服务**：应用内置 HTTP 服务监听局域网可达地址，电视端通过 `SetAVTransportURI` 拉取本机流。
- **投屏显示名称**：电视端显示的名称可自定义（如「xxx 投屏」），支持 `{title}` 占位符保留原标题；随时可改、下次投屏生效；投屏前也可在视频卡片上临时改名，仅本次生效。
- **设置页**：投屏显示名称、在线视频清晰度、代理（跟随系统 / 手动 / 直连）与站点登录状态（读取本机浏览器 / 应用登录浏览器 / 不使用）集中管理，见下文。
...
## 设置（投屏显示名称、代理与 Cookies）

点击右上角「⚙ 设置」进入设置页。清晰度、代理与 Cookies 只作用于**在线视频的解析与拉流**；设备发现与投屏流始终走局域网直连，不受影响。投屏显示名称只改变电视端显示的标题，不影响传输。设置持久化保存在系统用户配置目录下的 `AnyDLNA/config.json`（macOS：`~/Library/Application Support/AnyDLNA/`）。诊断日志在同目录的 `logs/anydlna.log`（解析耗时、直链条数、上游长度、投屏/跳转成败都在里面，出问题直接看它）。

### 投屏显示名称

投屏时电视上显示的名字来自视频标题（在线视频）或文件名（本地文件，不含扩展名）。在设置页填写「投屏显示名称」即可自定义：

| 填写内容 | 电视端显示 |
| --- | --- |
| 留空（默认） | 视频标题或文件名 |
| `xxx 投屏` | 固定显示 `xxx 投屏` |
| `{title} · 来自 AnyDLNA` | 原标题加上后缀，如 `某视频 · 来自 AnyDLNA` |

改动保存后对下一次投屏生效；投屏中跳转进度仍保持同一名称。此外，投屏前可在视频卡片上为本次单独填写名称（留空按上表规则），临时名称优先于全局设置、不落盘。

### 在线视频清晰度

限制在线视频的分辨率上限（本地文件投屏不受影响）：

| 档位 | 说明 |
| --- | --- |
| **最高可用**（默认） | 不限制分辨率，按站点可用的最高清晰度选取 |
| **最高 1080p / 720p / 480p** | 分辨率不超过该档位；码率显著降低，弱网下更流畅 |

编码偏好不变：仍优先 H.264 视频 + AAC 音频以尽量免转码直通。修改后对下一次解析生效；解析缓存按清晰度分键，切换档位不会串用旧直链。
...
| **用应用登录浏览器** | 打开一个**独立 profile** 的浏览器窗口供你登录，再把登录状态交给应用；可在下拉中指定用哪台已检测到的浏览器（默认取第一个候选）。适合日常浏览器不是 Chromium 系、或希望把登录态与应用隔离的场景。 |
...
> 若播放仍不流畅，通常是网络带宽而非解码：4K 源本身码率可达 9 Mbps 以上，免转码虽不耗 CPU，但仍需把这些数据传到设备。可在设置页把在线视频清晰度限制到 1080p / 720p / 480p，或检查设备的 Wi-Fi 信号。

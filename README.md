...
- **在线视频投屏**：粘贴视频页面/流地址，`yt-dlp` 只负责解析出可直接播放的直链，下载、定位、合并全由 `ffmpeg` 直连直链完成，经本机中转给电视——电视端不需要支持该网站，也不受 DASH 分离流限制。支持 YouTube、Bilibili 等数千个站点与 m3u8/mp4 直链（部分站点 CDN 拒绝非浏览器客户端，自动改由 yt-dlp 下载经管道中转）。
...
- **播放控制**：播放 / 暂停 / 停止、进度跳转（在线模式跳转时 `ffmpeg` 用同一直链从新位置定位，无需重新下载；转码模式跳转时转码进程从新位置重启）、音量调节（RenderingControl）。

...
- **设置页**：投屏显示名称、代理（跟随系统 / 手动 / 直连）与站点登录状态（读取本机浏览器 / 应用登录浏览器 / 不使用）集中管理，见下文。
...
点击右上角「⚙ 设置」进入设置页。代理与 Cookies 只作用于**在线视频的解析与拉流**；设备发现与投屏流始终走局域网直连，不受影响。投屏显示名称只改变电视端显示的标题，不影响传输。设置持久化保存在系统用户配置目录下的 `AnyDLNA/config.json`（macOS：`~/Library/Application Support/AnyDLNA/`）。诊断日志在同目录的 `logs/anydlna.log`（解析耗时、直链条数、上游长度、投屏/跳转成败都在里面，出问题直接看它）。
...
> 若播放仍不流畅，通常是网络带宽而非解码：4K 源本身码率可达 9 Mbps 以上，免转码虽不耗 CPU，但仍需把这些数据传到设备。可改投较低清晰度的视频源（如同一视频的 1080p 版本；界面暂未提供清晰度选择），或检查设备的 Wi-Fi 信号。
